...
function Dashboard() {
  let {logout,userposts,userblogpost,userinfofetch,userinfo} = useContext(UserContext);
  ...
  const changeusername=()=>{
    ...
    axios.put(namechangeapi,body,config).then((res)=>{
      if(res.status==200){
        setNamesave(false)
        userinfofetch();
      }
  ...
  const UpdateUserPassword=(e)=>{
    ...
    axios.put(UpdatePassapi,body,config).then((res)=>{
      if(res.status==200){
        setPassword1("");
        setPassword2("");
        setOpen(true)
...
{namesave?<>
  <CircularProgress style={{height:'30px',width:'30px',marginTop:'30px',color:'green'}}/>
</>:<></>}
        ...
        marginTop:'20px',width:'60px',height:'30px',cursor:'pointer'}} type="submit">Done</button>
      
        </div>
        {open?<>
          <Alert onClose={() => {closemsg()}} style={{marginTop:'10px'}}>Password Updated!</Alert>
        </>:<></>}
        {erropen?<>
          <Alert onClose={() => {closemsgerror()}} style={{marginTop:'10px'}} severity="error">Password Not Updated!</Alert>
        </>:<></>}
        </form>
      </div>
        ...
        <h1>Posts</h1>
         ...
         {userblogpost.map((obj)=>(
                   ...
                   <div className='post_content'>
                    ...
                    <DeleteIcon style={{color:'#808080',cursor:'pointer'}} onClick={e=>deletepost(obj._id)}/>
