...
function Detailblog() { 
    ...
    useEffect(()=>{
      const getcommentapi=`http://localhost:5000/api/comments/fetchComments/${id}`
      const config = {
        headers: {
    ...
    const makecomment=()=>{
      const commentapi=`http://localhost:5000/api/comments/postComment/${id}`
      ...
      axios.post(commentapi,body,config).then((res)=>{
  ...
  return ( 
    ...
    <Button variant="outlined" style={{marginRight:'10px'}} onClick={cancelcomment}>cancel</Button>
    <Button variant="outlined" onClick={makecomment}>comment</Button>
    </div>
     </div>
    </div>
    ...
    <div className='comments_user'>
      {blogcomment.map((obj)=>(
             <>
               ...
               <span style={{fontSize:'15px',fontWeight:'bold',marginLeft:'5px',marginRight:'5px'}}>{obj.username}</span>
